...
class RevenuePersonal extends Component {
    constructor(props) {
        super(props);
    }
...
const mapStateToProps = (state) => {
    const arr = state.revenues.dataRevenue['1'] ? state.revenues.dataRevenue['1'].list : [];
    const obj = state.revenues.dataRevenue['1'] ? state.revenues.dataRevenue['1'].options : {};
    return {
        listRevenue: arr,
        options: obj,
...
const mapDispatchToProps = dispatch => {
    return {
        getList: (body) =>  dispatch(getList(body)),
    };
};
