...
class RevenueStaff extends Component {
    constructor(props) {
        super(props);
    }
...
const mapStateToProps = (state) => {
    const arr = state.revenues.dataRevenue['2'] ? state.revenues.dataRevenue['2'].list : [];
    const obj = state.revenues.dataRevenue['2'] ? state.revenues.dataRevenue['2'].options : {};
    return {
        listRevenue: arr,
        options: obj,
...
const mapDispatchToProps = dispatch => {
    return {
        getListStaff: (body) =>  dispatch(getListStaff(body)),
    };
};
