    ...
    renderItem = (item, length) => {
        ...
        return (
            <TouchableOpacity
                ...
                {
                    nameSelected === `(${item.item.code}) ${item.item.tradeName} - ${item.item.name}` ? (
                ...
                <Text style={{ marginLeft: 8, fontSize: 14, color: TxtColor, flex: 1 }}>
                    ({item.item.code}) {item.item.tradeName} - {item.item.name}
                </Text>
            </TouchableOpacity>
        )
    }
    onChangeText = (text) => {
        ...
        bankInfo && bankInfo.map(item => {
            if (
                ...
                item.tradeName.toUpperCase().indexOf(text.toUpperCase()) > -1 ||
