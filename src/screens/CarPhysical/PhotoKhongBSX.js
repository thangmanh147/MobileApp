...
function PhotoCar({ 
    ...
    const onOpenCamera = () => {
        Actions.execute('replace', 'TakePhotoPhysical', { action: 'FORM_SALES_INVOICE_CAR', active: 8 })
    };
    ...
    return (
        ...
        <View style={styles.container}>
            ...
            <View style={{ marginTop: -25, flex: 1, zIndex: 100 }}>
                ...
                >
                    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                        {
                            (arrayCarImg && arrayCarImg[0]?.saleAvoidPaper == "") ? (
                                <IconPaperSvg width={50} height={50} />
                            ) : (
                                <Image 
                                    source={{ uri: arrayCarImg[0]?.saleAvoidPaper }} 
                                    style={{
                                        resizeMode: 'cover',
                                        height: '100%',
                                        width: '100%',
                    ...
                    <View style={{ flex: 2.2, justifyContent: 'center', marginLeft: 16 }}>
                        ...
                        <Text style={{ marginTop: 4, fontSize: 14, color: (arrayCarImg && arrayCarImg[0]?.saleAvoidPaper == "") ? "#ED1C24" : "#00A651" }}>{arrayCarImg && arrayCarImg[0]?.saleAvoidPaper == "" ? 'Chưa chụp ảnh' : 'Đã chụp ảnh'}</Text>
                    </View>
            ...
            <View style={{ marginVertical: 12 }}>
                {/* <FooterButton >
                    <Button
                        disabled={(arrayCarImg && arrayCarImg[0]?.saleAvoidPaper == "") ? true : false}
                        label={'TIẾP TỤC'}
                        marginTop={10}
                        marginBottom={16}
                        onPress={() => {
                ...
                </FooterButton> */}
                {
                    nameApp.includes('IAGENT') ? (
                        <SendLinkContract setLoading={(value) => setLoading(value)} disabledBtn={(arrayCarImg && arrayCarImg[0]?.saleAvoidPaper == "")} />
                    ) : (
                        <FooterButton >
                            <Button
                                disabled={(arrayCarImg && arrayCarImg[0]?.saleAvoidPaper == "") ? true : false}
...
export default connect(mapStateToProps, {saveLogContract})(PhotoCar);
