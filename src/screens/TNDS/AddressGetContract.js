...
class AccidentInsuranceBuyerInfo extends Component {
  ...
  handleCheck = async () => {
    ...
    // nếu chưa chọn => chọn và set thông tin
    if (isChecked == false) {
      await this.setState({
        isChecked: !isChecked,
      ...
      // nếu đã chọn => bỏ chọn và xóa thông tin
    } else {
      await this.setState({
        isChecked: !isChecked,
        fullNameReceive: '',
  ...
  validatePhoneNumber = () => {
    ...
    // check SĐT chỉ bao gồm 10 chữ số
  ...
  // validate required tỉnh/tp và quận/huyện form địa chỉ nhận ấn chỉ
  ...
  validateCompanyName = () => {
    // nếu chọn xuất hóa đơn VAT => bắt validate
    if (!this.state.VAT) {
      return true;
    }
    // required
  ...
  setIdProvince = data => {
    ...
    if (isReceive == true) {
      // nếu isRecive == true =>  set vào form địa chỉ nhận ấn chỉ
      this.setState({
      ...
      this.setState({
        // nếu isRecive == false =>  set vào form người mua
        province: data._name,
        provinceId: data.id,
        errorCodeBuyerProvince: '',
      });
...
import {saveCustomerInfoTnds} from './actions/tnds_buy';
