...
import { getListCommission } from '../actions';
import { dateImages } from '../assets';

class CommissionPersonal extends Component {
    constructor(props) {
        super(props);
    }
    ...
    componentDidMount() {
        new Store().getSession(Const.TOKEN).then(token => {
            const {listCommission, options, insuranceInfo} = this.props;
            if(listCommission.length === 0 && !options.dateType) {
    ...
    onSet = (objModal) => {
        const { getListCommission } = this.props;
        getListCommission(objModal);
    };
    ...
    renderToTal = () => {
        const {listCommission} = this.props;
        const arr = listCommission && listCommission.map(item => item?.commissionValue);
        return arr?.reduce((a, b) => Math.round(a) + Math.round(b), 0);
    };

    render() {
        const {listCommission, options} = this.props;
        return (
            <ScrollView style={{flex: 1}} contentContainerStyle={{marginHorizontal: 24}}>
                <View style={{flex: 1, alignItems: 'center', flexDirection: 'row', marginVertical: 16, justifyContent: 'flex-end'}}>
                    <ModalSelectTime onSet={this.onSet} idRoute={'1'} />
                </View>
                <Text style={{fontSize: 16, fontWeight: 'bold', color: TxtColor, textAlign: 'center'}}>HOA HỒNG</Text>
                ...
                {
                    listCommission && listCommission.map((item, index) => (
                        <View style={{
                            flexDirection: 'row',
                            paddingVertical: 12,
                            ...
                            <View style={{flex: 1}}>
                                <Text style={{color: TxtColor, fontSize: 14, textAlign: 'right'}}>{formatVND(Math.round(item?.commissionValue))}đ</Text>
                            </View>
                        </View>
                    ))
                }
...
const mapStateToProps = (state) => {
    const arr = state.commissions.dataCommission['1'] ? state.commissions.dataCommission['1'].list : [];
    const obj = state.commissions.dataCommission['1'] ? state.commissions.dataCommission['1'].options : {};
    return {
        listCommission: arr,
        options: obj,
...
const mapDispatchToProps = dispatch => {
    return {
        getListCommission: (body) =>  dispatch(getListCommission(body)),
    };
};
