...
import { getListCommissionStaff } from '../actions';
...
class CommissionStaff extends Component {
    ...
    onSet = (objModal) => {
        const { getListCommissionStaff } = this.props;
        getListCommissionStaff(objModal);
    };
        ...
        return (
            <ScrollView style={{flex: 1}} contentContainerStyle={{marginHorizontal: 24}}>
                <View style={{flex: 1, alignItems: 'center', flexDirection: 'row', marginVertical: 16, justifyContent: 'flex-end'}}>
                    <ModalSelectTime onSet={this.onSet} idRoute={'2'} />
                </View>
...
const mapStateToProps = (state) => {
    const arr = state.commissions.dataCommission['2'] ? state.commissions.dataCommission['2'].list : [];
    const obj = state.commissions.dataCommission['2'] ? state.commissions.dataCommission['2'].options : {};
    return {
        listCommission: arr,
        options: obj,
...
const mapDispatchToProps = dispatch => {
    return {
        getListCommissionStaff: (body) =>  dispatch(getListCommissionStaff(body)),
    };
};
