...
class AccidentBuyerInfo extends Component {
  ...
  getValueCom = () => {
    if (!nameApp.includes('EPTI') && !nameApp.includes('INSO')) {
      const { chosenPackage, orgCodeUser, idComSelected } = this.props;
      const { insuredCustomers } = this.state;
  ...
  handlePressTakePhoto = () => {
    Actions.Camera24h({familySecure: true});
  };
  ...
  storeInfomation = async values => {
    if (this.checkInsuredCustomers(values.isCompany)) {
      ...
      if (values.isVat) {
        ...
        buyerInfomation.vatCompanyAddress = values.vatCompanyAddress
      }
        ...
        <KeyboardAvoidingView
          ...
          <Animated.View style={{zIndex: headerZIndex}}>
            ...
            <View style={{
                ...
                <TouchableOpacity
                    onPress={() => Actions.PackageFaSecure()}
                    style={{
                        paddingHorizontal: 24,
                        paddingTop: 43,
                        zIndex: 2012
...
const mapStateToProps = (state, ownProps) => {
  ...
  return {
    ...
    ocrImg: state.VTAContract.ocrImg,
