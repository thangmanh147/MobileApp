...
import IconPaperSvg from '../../../../config/images/tab_carPhysical/IconPaperSvg';
...
function PhotoCar({
    ...
    const onOpenCamera = () => {
        Actions.execute('replace', 'PreviewTakePhotoPhysical', { action: 'FORM_SALES_INVOICE_CAR', active: 8, contractId: contractId })
    };
      
    const renderTextStatus = () => {
        if (
            dataContract?.contract_car?.contract_car_images?.filter(item => item?.fileKey == 'GIAY_BAN_GIAO_XE' && item?.status === 'tu-choi' && !dataPreviewCarImg[item?.fileKey])?.length > 0
        ) {
            return 'Từ chối ảnh'
        } else if (
            dataContract?.contract_car?.contract_car_images?.filter(item => item?.fileKey === 'GIAY_BAN_GIAO_XE' && item?.status === 'tu-choi' && dataPreviewCarImg[item?.fileKey])?.length > 0
    ...
    const renderStatus = () => {
        if (dataContract?.contract_car?.contract_car_images?.find(item => item?.fileKey === 'GIAY_BAN_GIAO_XE' && item?.status === 'tu-choi')) {
            return false;
        } else {
            return true;
        }
    ...
    const renderIcon = () => {
        return (
            <Image
                source={{ uri: dataContract?.contract_car?.contract_car_images?.find(item => item?.fileKey === 'GIAY_BAN_GIAO_XE')?.source || '' }} 
                style={{
                    resizeMode: 'cover',
                    height: '100%',
                    width: '100%',
    ...
    return (
        ...
        <View style={styles.container}>
            ...
            <View style={{ marginTop: -25, flex: 1, zIndex: 100 }}>
                <TouchableOpacity style={{
                    ...
                    onPress={() => requireLocation()}
                >
                    <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
                        {renderIcon()}
                    </View>
                    <View style={{ flex: 2.2, justifyContent: 'center', marginLeft: 16 }}>
                        <Text style={{fontSize: 14, color: TxtColor, fontWeight: 'bold'}}>Giấy bàn giao xe</Text>
                        <Text style={{ marginTop: 4, fontSize: 14, color: renderColor() }}>{renderTextStatus()}</Text>
                    </View>
...
export default connect(mapStateToProps)(PhotoCar);
