...
class TakePhotoPhysical extends Component {
  ...
  uploadImage = res => {
    ...
    const { active, action, contractId } = this.props;
  ...
  renderAction = (action) => {
    ...
    if (action == 'FORM_IMAGE_CAR') {
      return Actions.PreviewCarPhotoPhysical({contractId: contractId})
    } else if (action === 'FORM_CERTIFICATE_CAR' || action === 'FORM_REGISTRATION_CERTIFICATE_CAR') {
      Actions.PreviewPhotoCoBSX({contractId: contractId})
    } else if (action === 'FORM_SALES_INVOICE_CAR') {
      Actions.PreviewPhotoKhongBSX({contractId: contractId})
    } else {
      Actions.pop()
    }
  }
    ...
    return (
      <View style={styles.container}>
        ...
        {this.state.modalGuiToTakeCamera == false && (
          <TouchableOpacity
            onPress={() => this.renderAction(action)}
            style={{
              position: 'absolute',
              zIndex: 100,
              right: 0,
...
export default TakePhotoPhysical;
