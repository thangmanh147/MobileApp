...
class ModalNation extends React.Component {
  ...
  componentDidUpdate(prevProps: Readonly<P>, prevState: Readonly<S>, snapshot: SS) {
    const {areaId} = this.props;
    if(areaId !== 1 && areaId !== prevProps.areaId) {
        this.getData(areaId);
    }
}
  ...
  getData = (areaId) => {
    ...
    new Store().getSession(Const.TOKEN).then(token => {
      let url = `${URL}/api/master-data/v1/nations/list-nation?areaId=${areaId}&usedBy=2&limit=999`
      axios.get(url, {
        headers: {
...
export default connect(mapStateToProps)(ModalNation);
