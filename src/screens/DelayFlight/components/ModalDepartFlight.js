...
class ModalDepartFlight extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            newText: '',
    ...
    setItem = (data) => {
        this.props.setDepartFlight(data);
        this.props.onClosed();
    }
    renderItem = ({ item, index }) => {
        const { departSelected } = this.props;
        return (
            <View style={{ paddingHorizontal: 15, paddingBottom: 15, paddingTop: index === 0 ? 15 : 0 }}>
                ...
                >
                    {
                        departSelected?.flightNumber === item?.flightNumber ? (
    ...
    render() {
        ...
        const { onClosed, open, listDepart } = this.props;
        const arr = listDepart && listDepart.filter(item => item?.flightNumber?.toUpperCase().indexOf(newText.toUpperCase()) > -1)
        return (
            <ModalBox
                isOpen={open}
                entry={'bottom'}
...
export default ModalDepartFlight;
