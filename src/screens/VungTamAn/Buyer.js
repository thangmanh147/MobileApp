...
import PhoneInput from '../24hInsurance/ListInsureCustomer/PhoneInput';
import AddressInput from '../24hInsurance/ListInsureCustomer/AddressInput';
...
import { Color, colorBackground, colorPlaceholder, colorText, colorTitle, errValidColor, nameApp, NewColor, NewColorDisable, textDisable, TxtColor, URL } from '../../config/System';
...
class AccidentBuyerInfo extends Component {
  ...
  getValueCom = () => {
    if (!nameApp.includes('EPTI') && !nameApp.includes('INSO')) {
      ...
      const totalPrice = (chosenPackage?.priceByMonth[chosenPackage?.duration?.value] || 0) * insuredCustomers.length;
      new Store().getSession(Const.TOKEN).then(token => {
        const dataToken = jwt_decode(token);
        let url;
  ...
  storeInfomation = async values => {
    if (this.checkInsuredCustomers(values.isCompany)) {
      ...
      if (values.isVat) {
        ...
        buyerInfomation.vatCompanyTaxCode = values.vatCompanyTaxCode
      ...
      await this.props.saveInsuredCustomerInfoVTA(insuredCustomers);
      await this.props.saveBuyerInfomationVTA(buyerInfomation);
      await this.props.saveInsurancePrintAddressVTA(printAddress);
      contractLog.key = 'PreviewVTA';
      ...
      saveLogContract('A4', contractLog);
  ...
  logging = (insuredCustomers, values) => {
    ...
    this.props.logEventContract(
      `${nameApp}_VUNG_TAM_AN_EVENT_THONG_TIN_BEN_MUA`,
      {
        buyerInfo: values,
  ...
  validation = Yup.object().shape({
    ...
    vatCompanyName: Yup.string().when('isCompany', {
      is: false,
      then: Yup.string().trim().when('isVat', {
        is: true,
        then: Yup.string()
          .strict(false)
          // .trim()
    ...
    return (
      <View style={{ marginTop: 22 }}>
        ...
        >
          <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
            <View style={{ flex: 1.2 }}>
              <Text style={{ color: TxtColor, fontSize: 14, fontWeight: 'bold' }}>
                {chosenPackage?.name} {chosenPackage?.duration?.name}
              </Text>
              <Text style={{ color: TxtColor, fontSize: 14 }}>(Không chịu thuế VAT)</Text>
            </View>
            <View style={{ flex: 1 }}>
          ...
          <View style={{ justifyContent: 'space-between', flexDirection: 'row', marginVertical: 12 }}>
            <View style={{ flex: 1 }}>
              <Text style={{ color: TxtColor, fontSize: 14, fontWeight: 'bold' }}>
                Thanh toán
              </Text>
        ...
        {formik => (
          <View>
            ...
            <View>
              ...
              <View style={{ marginTop: 5 }}>
                <FormikInput
                  label={'Địa chỉ (Số nhà, tên đường, phường/xã) *'}
                  name={'buyerAddress'}
                  onChangeText={
            ...
            <Promotion
              totalPrice={(chosenPackage?.priceByMonth[chosenPackage?.duration?.value] || 0) * insuredCustomers.length}
              insurProductCode={'A4'}
            />
            ...
            <View>
              <View style={{ flexDirection: 'row', marginTop: 12 }}>
                <Text style={{ flex: 1, fontSize: 14, color: TxtColor }}>
      ...
      <View style={styles.container}>
        <KeyboardAvoidingView
          ...
          <Animated.View style={{zIndex: headerZIndex}}>
              <ImageHeaderScroll code={'A4'} offset={this.scrollYAnimatedValue} />
          </Animated.View>
            <View style={{
                position: 'absolute',
                top: 0,
          ...
          <View style={{zIndex: 2011}}>
              <HeaderScroll code={'A4'} offset={this.scrollYAnimatedValue} />
          </View>
          <ScrollView
            style={{ zIndex: 1000 }}
...
import {
  saveBuyerInfomationVTA,
  saveInsuredCustomerInfoVTA,
  saveInsurancePrintAddressVTA,
} from './actions/actionsVTA';
...
const mapStateToProps = (state, ownProps) => {
  return {
    ...
    promotionPrice: state.promotion.promotionPrice['A4'],
    contractLog: state.logContract.logContract['A4'] || {},
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    saveBuyerInfomationVTA: body => dispatch(saveBuyerInfomationVTA(body)),
    saveInsuredCustomerInfoVTA: body => dispatch(saveInsuredCustomerInfoVTA(body)),
    saveInsurancePrintAddressVTA: body =>
      dispatch(saveInsurancePrintAddressVTA(body)),
