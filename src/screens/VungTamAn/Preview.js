...
class AccidentPreviewInfo extends Component {
    ...
    onPay = async () => {
        ...
        const totalPrice = (chosenPackage?.priceByMonth[chosenPackage?.duration?.value] || 0) * insuredCustomerInfo.length;
        const payPrice = promotionPrice?.price ? totalPrice - promotionPrice?.price : totalPrice;
        let url = `${URL}/api/contract/v1/ml-vta-contracts`;
        let body = {
        ...
        Actions.Pay({ insurProductCode: 'A4' });
    };
    ...
    onLogging = (objLog, url) => {
        new Store().getSession(Const.TOKEN).then(token => {
            ...
            let _body = {
                "serviceName": `${nameApp}_PAY_VUNG_TAM_AN`,
                "extraData": objLog,
    ...
    render() {
        ...
        console.log('buyerInfomation', buyerInfomation)
        return (
                    ...
                    <View style={{
                        ...
                        <View style={{
                            ...
                            <View style={[styles.titleContainer, { justifyContent: 'space-between' }]}>
                                ...
                                <TouchableOpacity onPress={() => Actions.PackageVTA()}>
                                    <IconEditSvg width={16} height={15} />
                                </TouchableOpacity>
                            </View>
                            {/* render bảng phí  */}
