...
import { statusClaim } from './assets';
...
class DetailClaim extends Component {
    ...
    componentDidMount() {
        const { idClaim } = this.props;
        let url = `${URL}/api/ticket/v1/tickets/${idClaim}`;
                ...
                .then((res) => {
                    if (res.status === 200) {
                        return res.json().then(data => {
                            ...
                            this.setState({ itemClaim: data.data });
                        });
                    }
                })
                .catch(error => {
    ...
    takePhoto = (index) => {
        ImagePicker.openCamera({
            ...
            .then(image => {
                ...
                // };
                const { listImage } = this.state;
                listImage[index].value = { uri: image.path };
    ...
    selectPicture = (index) => {
        ImagePicker.openPicker({
            ...
            .then(image => {
                ...
                // };
                const { listImage } = this.state;
                listImage[index].url = image.path;
                listImage[index].title = `Ảnh ${index + 1}`;
                listImage[index].type = image.mime;
                this.setState({ listImage: listImage });
            })
            .catch();
    };
    ...
    render() {
        const { modalBenefit, listImage, showModal, itemClaim } = this.state;
        const FormikInput = handleTextInput(Input);
        const FormikSelect = handleTextInput(InputSelect);
        const statusItem = statusClaim[itemClaim.status] || statusClaim.default;
        return (
            <View style={styles.container}>
                ...
                <Formik
                    initialValues={{
                        benefitType: itemClaim.customerReason || '',
                        fullName: itemClaim.firstName || '',
                        phone: itemClaim.phoneNumber || '',
                        note: itemClaim.note || ''
                    }}
                            ...
                            <View style={{ flex: 1, paddingHorizontal: 24 }}>
                                ...
                                >
                                    ...
                                    <View style={{
                                        ...
                                        {
                                            itemClaim.image && itemClaim.image.map((item, index) => (
                                                <TouchableOpacity
                                                    disabled
                                                    onPress={() => this.selectPicture(index)}
                                                    style={{
                                                        marginRight: 15,
                                                        marginBottom: 15,
                                                        alignItems: 'center'
                                    ...
                                    <Text style={{ fontSize: 14, color: TxtColor }}>
                                        Trạng thái yêu cầu bồi thường: <Text style={{ color: statusItem.color }}>{statusItem.name}</Text>
                                    </Text>
                                </ScrollView>
                                {/* <Modal
                                    isVisible={showModal}
