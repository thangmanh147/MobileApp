...
class NewModalPlace extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            place: [
    ...
    setPlace = (data) => {
        this.props.setPlace(data)
        this.props.onClosed()
    }

    render() {
        const {fullData, place} = this.state
            ...
            >
                <View style={{
                    ...
                    <View style={{flex: 0.1,backgroundColor: '#F6F5F6',borderTopLeftRadius: 20, borderTopRightRadius: 20, marginTop: -10}}>
                        <Text style={{marginTop: 15, marginLeft: 40, fontWeight: '700', fontSize: 16}}>{this.props.title}</Text>
                    </View>
                        ...
                        <ScrollView keyboardShouldPersistTaps='always'>
                            {
                                place.length > 0 ? place.map((item, index) => {
                                    return (
                                        <TouchableOpacity onPress={() => this.setPlace(item)} style={{flexDirection: 'row',borderBottomWidth: 1,borderTopWidth: 0, borderColor: '#efefef',alignItems: 'center', paddingVertical: 10, paddingLeft: 0, marginHorizontal: 15}} key={index}>
...
export default connect(mapStateToProps)(NewModalPlace);
