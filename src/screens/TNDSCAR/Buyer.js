...
import {
    ...
    ERROR_REPRESENT_ROLE_REQUIRED, ERROR_COMPANY_NAME_FORMAT,
    ERROR_16_YEARS,
...
import { Color, colorPlaceholder, colorText, colorTitle, nameApp, NewColor, NewColorDisable, subColor, textDisable, TxtColor } from '../../config/System';
...
import {
    ...
    isTaxCode,
    subTimestamp,
    isIdentity, isPhoneAccident, isEmailAccident, isFullNameExpand,
} from '../../components/Functions';
import SimpleToast from 'react-native-simple-toast';
import ModalSex from '../FlightInsurance/component/ModalSex';
import FooterButton from '../../components/FooterButton';
import { saveBuyerCar } from './actions/tndsCarActions';
...
class BuyerCar extends Component {
    ...
    storeInfomation = async values => {
        const {saveBuyerCar, saveLogContract, contractLog} = this.props;
        let body = {
            isValid: true,
            infoBuyer: values,
    ...
    logging = (data) => {
        ...
        this.props.logEventContract(
            `${nameApp}_TNDS_OTO_EVENT_THONG_TIN_BEN_MUA`,
            {
                buyerInfo: data,
    ...
    setBuyerType = (formik, value) => {
        formik.setFieldValue('isCompany', value);
    };
    ...
    onAutoFill = async (formik) => {
        ...
        if (!values.checkedSameInfo) {
            const {dataProvince, dataDistrict} = this.state;
            ...
            await this.setState({
                dataProvinceReceiver: dataProvince,
                dataDistrictReceiver: dataDistrict
            });
        }
                    ...
                    return (
                        <View>
                            ...
                            <View style={{ flexDirection: 'row', marginTop: 12 }}>
                                <TouchableOpacity
                                    onPress={() => this.setBuyerType(formik, false)}
                                    style={{
                                        flexDirection: 'row',
                                        alignItems: 'center',
                                        flex: 1,
                                    ...
                                    <Text style={{ marginLeft: 8, color: TxtColor, fontSize: 14 }}>Cá nhân</Text>
                                </TouchableOpacity>
                                <View style={{width: 24}} />
                                <TouchableOpacity
                                    onPress={() => this.setBuyerType(formik, true)}
                                    style={{
                                        flexDirection: 'row',
                                        alignItems: 'center',
                                        flex: 1,
                                    ...
                                    <Text style={{ marginLeft: 8, color: TxtColor, fontSize: 14 }}>Doanh nghiệp</Text>
                                </TouchableOpacity>
                            </View>
                            <View>
                                {
            ...
            <View style={styles.container}>
                <KeyboardAvoidingView
                    ...
                    <Animated.View style={{zIndex: headerZIndex}}>
                        <ImageHeaderScroll code={'C1'} offset={this.scrollYAnimatedValue} />
                    </Animated.View>
                    <View style={{
                        position: 'absolute',
                        top: 0,
                        ...
                        <TouchableOpacity
                            onPress={() => Actions.PackageTNDSCar()}
                            style={{
                    ...
                    <View style={{zIndex: 2011}}>
                        <HeaderScroll code={'C1'} offset={this.scrollYAnimatedValue} />
                    </View>
                    <ScrollView
                        style={{ zIndex: 1000 }}
...
const mapStateToProps = state => ({
    buyer: state?.tndsCar.buyer,
    infoBuyer: state?.tndsCar?.buyer?.infoBuyer,
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        saveBuyerCar: body => dispatch(saveBuyerCar(body)),
