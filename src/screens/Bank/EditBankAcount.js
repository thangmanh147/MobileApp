...
class AddBank extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            fullName:this.props.data.customer_full_name,
            acountNumber:this.props.data.bank_account,
            bankName:this.props.data.bank_name,
            listBank: [
                {
                    ...
                    <View style={styles.blockBackground}>
                        <TouchableOpacity onPress={()=> this.openModalListBank()} style={{flex:1,flexDirection:'row',borderBottomWidth:1,borderColor:'#D9D9D9',marginHorizontal:10}}>
                            ...
                            <View style={{justifyContent:'flex-end', flex: 1,alignItems:'center',flexDirection:'row'}}>
                                <Text style={{fontSize:14,color:'#414042',paddingRight:10}}>{this.state.bankName == '' ? data.bank_name : this.state.bankName}</Text>
                        ...
                        <View style={{flex:0.7,borderBottomWidth:1,borderColor:'#D9D9D9',marginHorizontal:10,flexDirection:'row'}}>
                            ...
                            <View style={{flex:1,justifyContent:'center',alignItems:'flex-end'}}>
                                <TextInput value={data.bank_account} onChangeText={text => this.onChangeText(text)} style={{fontSize:14,width:'100%', paddingVertical: 10 ,textAlign: 'right'}} />
                            </View>
                        </View>
                        <View style={{flex:0.7,borderBottomWidth:1,borderColor:'#D9D9D9',marginHorizontal:10,flexDirection:'row'}}>
                            <View style={{flex:1,justifyContent:'center'}}>
                            ...
                            <View style={{flex:1,justifyContent:'center',alignItems:'flex-end'}}>
                                <TextInput value={data.customer_full_name} onChangeText={text => this.onChangeName(text)} style={{fontSize:14,width:'100%',paddingVertical: 10 ,textAlign: 'right'}} />
                            </View>
                        </View>
                        <View style={{flex:1.3,paddingHorizontal:10}}>
                            <TouchableOpacity onPress={()=>Actions.DealSuccess()}
                                              ...
                                              style={[styles.buttonAdd,{backgroundColor:this.state.moneyInput !== '' && this.state.fullName !== '' && this.state.bankName !== '' ? Color :'#d9d9d9',}]}>
                                <Text style={{fontSize:15,fontWeight:'bold',color:'#ffffff'}}>LƯU THÔNG TIN</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
